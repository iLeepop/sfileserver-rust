...
use crate::http::generate_directory_listing;
...
impl Server {
    ...
    pub async fn run(&self) {
        ...
        loop {
            ...
            tokio::spawn(handle_connect(socket, directory));
        }
    }
}

async fn handle_connect(mut socket: TcpStream, directory: String) {
    ...
    if file_path.exists().await {
        if file_path.is_dir().await {
            match generate_directory_listing(&file_path, path).await {
                Ok(html) => {
                    let header = format!(
